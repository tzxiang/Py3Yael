...
import sys,time,inspect,_thread,threading,os,errno,types,traceback, random
...
the_allocate_lock=_thread.allocate_lock

def alt_allocate_lock():
  return VerbLock(the_allocate_lock())

# _thread.allocate_lock=alt_allocate_lock
...
class Pool:
  ...
  def __init__(self,cons):
    self.cons=cons
    self.plock=_thread.allocate_lock()
    self.pool=[]
...
class ParallelMap:
   ...
   def __init__(self,n,l,f):
     self.orig_f=f
     self.result=[]
     self.reslock=_thread.allocate_lock()
...
class ParallelIter:
  ...
  def __init__(self,n,l,f):
    ...
    # protect producer
    self.src_lock = _thread.allocate_lock()
    self.stop=False
    self.k_out=0
    self.results={}
    self.n_run=n
